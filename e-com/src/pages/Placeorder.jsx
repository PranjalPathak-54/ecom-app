...
const Placeorder = () => {
  ...
  const {navigate,backendUrl,token,setcart,cart,cartTotal,delivery_fee,products}=useContext(ShopContext)
    ...
    try{
      ...
      switch(method){
        case 'cod':
          ...
          break;
        case 'stripe':
          const striperesponse=await axios.post(backendUrl + '/api/order/stripe',orderData,{headers:{token: token}})
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <input required onChange={onChangeHandler} name='email' value={formData.email} className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type="email" placeholder='Email Addreess'/>
