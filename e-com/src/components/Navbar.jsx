...
const Navbar = () => {
  ...
  const {setshowsearch,cartCount,navigate,token,settoken,setcart}=useContext(ShopContext);
  const logout=()=>{
    navigate('/login')
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
      ...
      <div className='flex items-center gap-6'>
        ...
        <img onClick={()=>setisvisible(true)} src={assets.menu_icon} className='w-5 cursor-pointer'/>
      </div>
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${isvisible?'w-full':'w-0'}`}>
        <div className='flex flex-col text-gray-600'>
            ...
            <NavLink  onClick={()=>setisvisible(false)} className='py-2 pl-6 border' to='/collections'>COLLECTION</NavLink>
